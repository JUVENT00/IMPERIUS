...
// ============================================================
const { getJogador, salvarJogador, adicionarConquista, adicionarTitulo, adicionarXP, adicionarMoedas, getCasamento, setCasamento, deleteCasamento, formatarBelarium, formatarPreco, registrarHistorico } = require('./db');
...
function entrarMasmorra(jogador_id, nome_masmorra) {
  ...
  let resultado_texto = '';
  if (sobreviveu) {
    j.masmorras_concluidas = (j.masmorras_concluidas || 0) + 1;
    salvarJogador(jogador_id, j); // persiste o cooldown antes de aplicar XP/moedas
    const nivel_result = adicionarXP(jogador_id, xp_ganho);
    adicionarMoedas(jogador_id, moedas_ganhas);
    registrarHistorico(jogador_id, `🏰 Concluiu a masmorra ${masmorra.nome} (+${xp_ganho} XP)`);
    if (j.masmorras_concluidas >= 5) adicionarTitulo(jogador_id, 'sobrevivente');
    resultado_texto = `${BORDAS.topo}\n   🏰 MASMORRA CONCLUÍDA!\n${BORDAS.baixo}\n\n${BORDAS.linha}\n║ ✅ ${j.nome} sobreviveu!\n║ 🏰 ${masmorra.nome}\n${BORDAS.meio}\n║ 📊 RECOMPENSAS:\n║ ⭐ XP: +${xp_ganho}\n║ 💰 +${formatarPreco(moedas_ganhas)}\n${nivel_result?.subiu ? `${BORDAS.meio}\n║ 🎉 SUBIU PARA NÍVEL ${nivel_result.nivelDepois}!\n` : ''}${BORDAS.fim}`;
  } else {
    j.hp = Math.max(1, Math.floor(j.hp_max * 0.1));
    salvarJogador(jogador_id, j);
    registrarHistorico(jogador_id, `💀 Foi derrotado na masmorra ${masmorra.nome}`);
...
function acampar(jogador_id) {
  ...
  const agora = Date.now();
  if (j.cooldown_acampar && agora - j.cooldown_acampar < 3 * 60 * 1000) {
    const restante = Math.ceil((3 * 60 * 1000 - (agora - j.cooldown_acampar)) / 60000);
    return `${BORDAS.topo}\n⏳ Você já acampou recentemente!\nAguarde ${restante} minuto(s).\n${BORDAS.baixo}`;
  }
...
function aceitarCasamento(jogador_id, proponente_id, proponente_nome, custo) {
  ...
  salvarJogador(jogador_id, j);
  registrarHistorico(proponente_id, `💍 Se casou com ${j.nome}`);
  registrarHistorico(jogador_id, `💍 Se casou com ${proponente_nome}`);
