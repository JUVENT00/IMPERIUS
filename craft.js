...
// ============================================================
const { getJogador, salvarJogador, formatarBelarium, formatarPreco } = require('./db');
...
function verReceitas() {
  const linhas = Object.entries(RECEITAS).map(([key, r]) => {
    ...
    return `🔧 */craft ${key}*\n   ↳ ${r.nome}\n   ↳ Precisa: ${mats} + ${formatarPreco(r.moedas)}`;
  });
  return linhas;
}
...
function craftar(jogador_id, nome_receita) {
  ...
  if ((j.moedas || 0) < receita.moedas) {
    return { erro: `❌ Você precisa de ${formatarPreco(receita.moedas)} pra craftar isso! Você tem ${formatarBelarium(j)}.` };
  }
