...
const { responderComoImperius, mencionaImperius, podeUsarChat } = require('./imperius_ai');
...
const ALIASES = {
  ...
  'armaduras': ['armaduras', 'armadura', 'armasa'],
...
async function processarMensagem(msg) {
  ...
  if (!texto.startsWith(PREFIX)) {
    ...
    // Chat livre: se a mensagem citar "Imperius", responde em personagem
    // usando perguntas e respostas prontas — sem nenhuma API paga.
    if (!criando.has(from) && !batalhaAtiva.has(from) && mencionaImperius(texto)) {
      if (!podeUsarChat(from)) return;
      const resposta = responderComoImperius(texto);
