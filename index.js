...
const { CLASSES, ARMAS, ARMADURAS, ARMAS_PRIMORDIAIS, REGIOES, ITENS_LOJA, NIVEIS, calcularNivel, TITULOS, CONQUISTAS } = require('./gameData');
...
function checarTituloImperador(jogador_id) {
  ...
  if (!j) return;
  const nivel_info = calcularNivel(j.xp || 0);
  if (nivel_info.nivel >= 100) db.adicionarTitulo(jogador_id, 'imperador');
}
...
${S.baixo}`;
  ...
  if (cmd === 'setnivel') {
    ...
    let xp_necessario = 0;
    for (let i = 0; i < nivel_alvo - 1 && i < NIVEIS.length; i++) xp_necessario += NIVEIS[i].xp;
    j_niv.xp = xp_necessario;
    db.salvarJogador(alvo_id, j_niv);
    return enviar(jid, bloco('⭐ NÍVEL ALTERADO 【⭐】', [`*${j_niv.nome}* agora está no nível *${nivel_alvo}*!`]), [alvo_id]);
  }
  ...
  if (batalha.monstro_hp <= 0) {
    ...
    if (isBoss) {
      ...
      if (batalha.boss.drop_arma && ARMAS_PRIMORDIAIS.length > 0) {
        ...
        drop_texto = `🎁 *Drop!* ${batalha.boss.drop}`;
      }
    } else {
