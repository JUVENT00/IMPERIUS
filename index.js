...
async function gerarImagemPersonagem(classe, nome, genero, caracteristicas, historia) {
  try {
    const https = require('https');
    const prompt = encodeURIComponent(
      `fantasy RPG character portrait, ${genero}, ${classe} class, ${caracteristicas}, ` +
      `epic dark fantasy art style, dramatic lighting, detailed, cinematic, ` +
      `black background, high quality digital art, game character design`
    );
...
async function processarCriacao(from, jid, texto, msg) {
  ...
  if (estado.etapa === 'historia') {
    ...
    // Avisar que está gerando imagem
    await enviar(jid, `🎨 _Gerando sua imagem de personagem..._
_Aguarde alguns segundos!_`);

    // Gerar imagem com IA
    const img_buffer = await gerarImagemPersonagem(
      classeData.nome, nome, genero || 'outro', caracteristicas || '', historia
    );
