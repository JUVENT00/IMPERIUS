...
// ── SEGURANÇA: Limpar batalhas travadas a cada 10 minutos ──
setInterval(() => {
  const agora = Date.now();
  for (const [id, batalha] of batalhaAtiva.entries()) {
    if (agora - batalha.inicio > 10 * 60 * 1000) { // 10 minutos
...
${S.baixo}`;
  ...
  if (cmd === 'renascer') {
    ...
    if (j_r.cooldown_renascer && agora_r - j_r.cooldown_renascer < 10 * 60 * 1000) {
      const restante_r = Math.ceil((10 * 60 * 1000 - (agora_r - j_r.cooldown_renascer)) / 60000);
    ...
    if (chance_r > 70) {
      ...
      return enviar(jid, bloco('☠️ NEGADO 【❌】', [
        ...
        '━━━━━━━━━━',
        '_Tente novamente em 10 minutos_'
      ]));
    }
