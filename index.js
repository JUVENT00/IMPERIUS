...
const ALIASES = {
  ...
  'verperfil': ['verperfil', 'verperfi', 'vperfil', 'vperfi'],
...
async function enviar(jid, texto, mencoes = [], quotado = null) {
  try {
    const opcoes = quotado ? { quoted: quotado } : {};
    await sock.sendMessage(jid, { text: texto, mentions: mencoes }, opcoes);
  }
...
${S.baixo}`;
  ...
  if (cmd === 'rpg') {
    return enviar(jid, bloco('𝐈𝐌𝐏𝐄𝐑𝐈𝐔𝐒 【📋】', [
      ...
      '  ↳ /verperfil @numero — Ver ficha de outro jogador',
  ...
  if (cmd === 'verperfil') {
    ...
    if (!alvo) return enviar(jid, `❌ Mencione quem você quer ver! Ex: /verperfil @numero`, [], msg);
    const j = db.getJogador(alvo);
    if (!j) return enviar(jid, `❌ Essa pessoa não tem personagem!`, [], msg);
    ...
    const perfil_txt = bloco('𝐏𝐄𝐑𝐅𝐈𝐋 𝐃𝐄 【👤】', [
        `👤 ${j.nome} ${j.imperador ? '👑' : ''}`,
        `🏷️ ${j.titulo_ativo || 'Sem título'}`,
        `🎭 ${CLASSES[j.classe]?.nome || j.classe}`,
        `⭐ Nível: ${nivel} | Rank: ${j.rank || 'F'}`,
    ...
    return enviar(jid, perfil_txt, [alvo], msg);
  }
