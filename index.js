...
const { EXPLICACOES, buscarExplicacao } = require('./explicacoes');
...
const ALIASES = {
  ...
  'explicar': ['explicar', 'explica', 'explicarme'],
  'comandosadm': ['comandosadm', 'meuscomandos', 'oquepossofazer'],
...
function gerarOfertasMercador(jogador_id) {
  ...
  const pool = [
    ...
    ...ITENS_LOJA.filter(i => i.preco > 0 && !i.exclusiva).map(i => ({ tipo: 'item', id: i.id, nome: i.nome, preco: i.preco, raridade: null })),
  ];
  const escolhidos = [];
...
async function processarMensagem(msg) {
  ...
  if (!texto.startsWith(PREFIX)) {
    ...
    if (!criando.has(from) && !batalhaAtiva.has(from) && (mencionaImperius(texto) || ehGrupoIniciantes)) {
      ...
      // No grupo de iniciantes, primeiro tenta achar uma explicação de
      // verdade sobre o que a pessoa perguntou (drop, loja, boss, pet...)
      // antes de cair na resposta genérica da IA.
      if (ehGrupoIniciantes) {
        const palavrasMsg = texto.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '').replace(/[^a-z\s]/g, '').split(/\s+/);
        for (const p of palavrasMsg) {
          const exp = buscarExplicacao(p);
          if (exp) { await enviar(jid, bloco(exp.titulo, exp.texto)); return; }
        }
      }
...
` + bloco('𝐆𝐄𝐑𝐀𝐋 【📋】', ['ℹ️ /info', '🆘 /ajuda', '🆕 /expiniciantes', '📖 /explicar [tema]']) + `
...
${S.baixo}`;
  ...
  // ── EXPLICAR (base de conhecimento de comandos/mecânicas) ──
  if (cmd === 'explicar') {
    if (!resto) {
      const topicos = Object.keys(EXPLICACOES).sort().join(', ');
      return enviar(jid, bloco('📖 /EXPLICAR 【📖】', [
        'Use: /explicar [comando ou termo]',
        'Ex: /explicar loja',
        'Ex: /explicar drop',
        '━━━━━━━━━━',
        'Temas disponíveis:',
    ...
    const explicacao = buscarExplicacao(resto);
    if (!explicacao) {
      return enviar(jid, `❌ Não tenho explicação pra *"${resto}"* ainda.\n\nUse */explicar* sem nada pra ver os temas disponíveis, ou */ajuda* pra ver todos os comandos.`);
    }
    return enviar(jid, bloco(explicacao.titulo, explicacao.texto));
  }

  // ── COMANDOS ADM (o que um ADM pode/não pode fazer) ─────────
  if (cmd === 'comandosadm') {
    if (!isDono(from) && !isADM(from)) {
      return enviar(jid, `❌ Você não é ADM. Só o Deus concede esse cargo com /darADM.`);
    }
    return enviar(jid, bloco('⚖️ PODERES DE ADM 【⚖️】', [
      '✅ Você PODE:',
      '🕊️ /revivernpc @jogador — reviver',
      '   quem morreu, com HP e mana cheios',
      '━━━━━━━━━━',
      '❌ Você NÃO PODE (só o Deus):',
      '⚔️ Dar ou tirar armas/itens',
      '💰 Dar ou tirar Belarium',
      '⚖️ Nomear ou remover outros ADMs',
      '☠️ Usar poderes de Deus (abençoar,',
      '   amaldiçoar, matar, eventos divinos)',
      '━━━━━━━━━━',
      '_Seu papel é cuidar de quem tá caído_',
      '_em batalha e ajudar os iniciantes._'
    ]));
  }
  ...
  if (cmd === 'itens') {
    ...
    const IDS_ESPECIAIS = ['antidoto','purificador','elixir_forca','pedra_ressurreicao','amuleto_sorte','elixir_nivel'];
  ...
  if (isDono(from)) {
    const CMDS_LIVRES = ['deus','adm','aceitardeus','ignorardeus','deusdescansar',
      ...
      'resetcd','dartitulo','givedangerarmor','givenakanoarmor','giveescarlatearmor','givealabardaarmor','removeritem','daradm','removeradm','listaadm','definirgrupoiniciante','removergrupoiniciante','explicar','comandosadm','verperfil'];
  ...
  if (cmd === 'revivernpc') {
    ...
    // Sem menção = reviver a si mesmo (no WhatsApp não dá pra se auto-marcar)
...
async function aplicarDanoHabilidadeNaBatalha(from, jid, resultado) {
  ...
  if (batalha.monstro_hp <= 0) {
    ...
    if (isBoss) {
      ...
      // ── Drop raro de itens exclusivos de Boss (Poção de XP / Pergaminho de Teletransporte) ──
      const ITENS_RAROS_BOSS = ['pocao_xp', 'pergaminho_teletransporte'];
      if (rand(1, 100) <= 4) {
        const idRaro = ITENS_RAROS_BOSS[Math.floor(Math.random() * ITENS_RAROS_BOSS.length)];
        const itemRaro = ITENS_LOJA.find(i => i.id === idRaro);
        if (itemRaro) {
          if (!j.inventario) j.inventario = [];
          j.inventario.push(itemRaro.id);
          const extra_raro = `✨ *Drop raríssimo!* Você recebeu *${itemRaro.nome}*!`;
          drop_texto = drop_texto ? `${drop_texto}\n${extra_raro}` : extra_raro;
        }
      }
    } else {
