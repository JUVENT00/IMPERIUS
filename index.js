...
${S.baixo}`;
  ...
  if (cmd === 'confirmarroleta') {
    const img_roleta = require('path').join(__dirname, 'roleta.jpg');
    if (require('fs').existsSync(img_roleta)) {
      try {
        await sock.sendMessage(jid, {
          image: require('fs').readFileSync(img_roleta),
          caption: '🎰 _A roleta gira..._',
          mimetype: 'image/jpeg'
        });
      } catch(e) {}
    }
