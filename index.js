...
const { renascer, suicidar, reviverPorNecromante, aprovarAcaoServo, liberarServo } = require('./death');
...
${S.baixo}`;
  ...
  if (cmd === 'tentardomar') {
    ...
    if (ehAnimalSelvagem(nome_monstro_dom)) {
      if (j_dom.animal) return enviar(jid, bloco('❌ ERRO 【⚠️】', [
        `Você já tem um animal domado (${j_dom.animal.nome})!`,
    ...
    if (criaturas_regiao.length === 0) return enviar(jid, `❌ Não há criaturas domáveis nesta região!`);

    let escolhida = criaturas_regiao.find(([_, c]) => c.nome.toLowerCase().includes(nome_monstro_dom.toLowerCase()));
    if (!escolhida) escolhida = criaturas_regiao[Math.floor(Math.random() * criaturas_regiao.length)];
  ...
  if (cmd === 'chamarpet' && batalhaAtiva.has(from)) {
    ...
    if (b_ataque.monstro_hp <= 0) {
      ...
      return enviar(jid, bloco('𝐕𝐈𝐓𝐎́𝐑𝐈𝐀 【🏆】', [
        `${r_ataque.emoji} ${r_ataque.nome} atacou e derrotou ${b_ataque.monstro_nome}!`,
        `💥 Dano: ${r_ataque.dano}`,
        '━━━━━━━━━━',
        `⭐ XP: +${xp_ganho}`,
    ...
    return enviar(jid, bloco('𝐁𝐀𝐓𝐀𝐋𝐇𝐀 — 𝐓𝐔𝐑𝐍𝐎 【⚔️】', [
      `${r_ataque.emoji} ${r_ataque.nome} atacou! Dano: *${r_ataque.dano}*`,
      '━━━━━━━━━━',
