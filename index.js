...
const { batalharMonstro, batalharBoss, pvp, usarHabilidade, usarUltimate, rolarD20, rand, gerarMonstro, gerarBoss, faseAtualBoss, calcularDanoBase, calcularDefesa, aplicarPassivaClasse, getResultadoD20, usarHabilidadeArma, usarSupremaArma } = require('./combat');
...
const { verRanking, verRankingConquistas, verConquistas, verTitulos, usarTitulo, verMissoes, progredirMissao, coletarMissao, matarJogador, darItem, abencoarJogador, maldicionarJogador, eventoGlobal, statusBot } = require('./events');
...
function gerarListaArmasPorTipo(tipo) {
  ...
  const raridades = [...new Set(compraveis.map(a => a.raridade))];
  const linhas = [];
  raridades.forEach(r => {
    const doTier = compraveis.filter(a => a.raridade === r);
    const exemplo = doTier[Math.floor(Math.random() * doTier.length)];
    linhas.push(`*${exemplo.nome}* — 🥉 ${exemplo.preco} Bronze | Dano: ${exemplo.dano[0]}-${exemplo.dano[1]} | ${r}`);
  });
  linhas.push('━━━━━━━━━━');
  ...
  linhas.push('🛒 */comprar [nome exato]* — comprar');
  return linhas;
}
...
async function processarMensagem(msg) {
  ...
  if (!texto.startsWith(PREFIX)) {
    ...
    if (!criando.has(from) && !batalhaAtiva.has(from) && mencionaImperius(texto)) {
...
${S.baixo}`;
  ...
  // ── INFOARMAS ─────────────────────────────────────────────
  if (cmd === 'armaduras') {
    const linhas = ARMADURAS.map(a => `*${a.nome}* — 🥉 ${a.preco} Bronze | Defesa: +${a.defesa} | ${a.raridade}`);
    return enviar(jid, bloco('𝐀𝐑𝐌𝐀𝐃𝐔𝐑𝐀𝐒 【🛡️】', [
      ...linhas,
      '━━━━━━━━━━',
      '🛒 */comprar [nome]* — comprar',
  ...
  if (cmd === 'buscararma') {
    ...
    const linhas = encontradas.map(a => `*${a.nome}* — 🥉 ${a.preco} Bronze | Dano: ${a.dano[0]}-${a.dano[1]} | ${a.raridade}`);
  ...
  if (cmd === 'atacar') {
    ...
    if (resultado.erro) return enviar(jid, resultado.erro);
    if (resultado.atacante_venceu) progredirMissao(from, 'pvp');
  ...
  if (cmd === 'itens') {
    ...
    if (sub.includes('poc') || sub.includes('poç')) {
      const linhas = ITENS_LOJA.filter(i => IDS_POCOES.includes(i.id)).map(i => `${i.nome} — 🥉 ${i.preco} Bronze`);
      return enviar(jid, bloco('𝐏𝐎𝐂̧𝐎̃𝐄𝐒 【🧪】', [...linhas, '', '💡 /comprar [nome]']));
    }
    if (sub.includes('especial')) {
      const linhas = ITENS_LOJA.filter(i => IDS_ESPECIAIS.includes(i.id)).map(i => `${i.nome} — 🥉 ${i.preco} Bronze`);
  ...
  if (cmd === 'comida') {
    return enviar(jid, bloco('𝐂𝐎𝐌𝐈𝐃𝐀 【🥩】', [
      '🥩 CARNES',
      '  🥩 Carne Crua (#CCC) — 🥉 30 Bronze',
      '  🥩 Carne Fresca (#CCF) — 🥉 60 Bronze',
      '  🥩 Carne Assada (#CCA) — 🥉 80 Bronze',
      '  🥩 Carne Rara (#CCR) — 🥉 150 Bronze',
      '  🥩 Carne de Dragão (#CCD) — 🥉 400 Bronze',
      '  🥩 Carne Sagrada (#CCS) — 🥉 600 Bronze',
      '',
      '🍯 MÉIS E DOCES',
      '  🍯 Mel Simples (#CMS) — 🥉 50 Bronze',
      '  🍯 Mel Dourado (#CMD) — 🥉 500 Bronze',
      '  🍯 Mel Sagrado (#CMG) — 🥉 700 Bronze',
      '  🌟 Néctar dos Deuses (#CNE) — 🥉 900 Bronze',
      '  ✨ Ambrosia (#CAN) — 🥉 1200 Bronze',
      '',
      '🌿 ERVAS E ESSÊNCIAS',
      '  🌿 Erva do Bosque (#CEB) — 🥉 40 Bronze',
      '  🌿 Erva Ancestral (#CEA) — 🥉 120 Bronze',
      '  🌱 Raiz Ancestral (#CRA) — 🥉 200 Bronze',
      '  ✨ Essência Mágica (#CEM) — 🥉 500 Bronze',
      '  ✨ Essência Primordial (#CEP) — 🥉 1000 Bronze',
      '',
      '🍎 FRUTAS MÍTICAS',
      '  🫐 Baga Espiritual (#CFB) — 🥉 80 Bronze',
      '  🍑 Figo Sagrado (#CFF) — 🥉 500 Bronze',
      '  🍎 Romã do Hades (#CFR) — 🥉 700 Bronze',
      '  🍎 Maçã Dourada (#CFM) — 🥉 1000 Bronze',
      '  🌟 Fruto da Imortalidade (#CFI) — 🥉 1500 Bronze',
      '',
      '🌊 AQUÁTICOS',
      '  🌊 Alga Abissal (#CAL) — 🥉 150 Bronze',
      '  🪸 Coral Sagrado (#CCO) — 🥉 250 Bronze',
      '  🐟 Peixe Espectral (#CPE) — 🥉 180 Bronze',
      '  🐍 Escama de Leviatã (#CEL) — 🥉 700 Bronze',
      '',
      '💎 MINERAIS',
      '  💎 Pedra de Luz (#CPL) — 🥉 100 Bronze',
      '  🔷 Cristal de Gelo (#CCG) — 🥉 200 Bronze',
      '  ⭐ Minério Sagrado (#CMI) — 🥉 450 Bronze',
      '',
      '💡 /comprar #CCA'
    ]));
  }
  ...
  if (cmd === 'negociar') {
    ...
    if (j_neg.moedas < oferta.preco) return enviar(jid, `❌ Belarium insuficiente! Você tem ${db.formatarBelarium(j_neg)}, precisa de ${oferta.preco} 🥉 Bronze.`);
    ...
    return enviar(jid, bloco('🧳 NEGÓCIO FECHADO 【✅】', [
      `Você comprou *${oferta.nome}* por *${oferta.preco}* 🥉 Bronze!`,
  ...
  if (isDono(from)) {
    const CMDS_LIVRES = ['deus','adm','aceitardeus','ignorardeus','deusdescansar',
      ...
      'resetcd','dartitulo','givedangerarmor','givenakanoarmor','verperfil'];
  ...
  // ── REVIVER NPC / JOGADOR (poder do Deus) ──────────────────
  if (cmd === 'revivernpc') {
    const alvo_id = extrairMencao(resto, msg);
...
async function aplicarDanoHabilidadeNaBatalha(from, jid, resultado) {
  ...
  if (batalha.monstro_hp <= 0) {
    ...
    db.adicionarXP(from, xp_ganho);
    checarTituloImperador(from);
    const moedas_creditar = isBoss ? moedas * 1000 : moedas;
    db.adicionarMoedas(from, moedas_creditar);
    ...
    return enviar(jid, bloco(isBoss ? '𝐁𝐎𝐒𝐒 𝐃𝐄𝐑𝐑𝐎𝐓𝐀𝐃𝐎 【🏆】' : '𝐕𝐈𝐓𝐎́𝐑𝐈𝐀 【🏆】', [
      ...
      `⭐ XP: +${xp_ganho}`,
      isBoss ? `💰 +${moedas} 🥈 Prata` : `💰 +${moedas} 🥉 Bronze`,
  ...
  if (resultado.efeito?.tipo === 'paralisia' || tick.pular_contra_ataque) {
    ...
    if (isBoss) {
      const fase = faseAtualBoss(batalha.boss);
    ...
    dano_monstro = Math.max(3, dano_bruto_monstro - Math.floor(defesa_jogador * 0.5));
  }
...
async function recompensarParceirosCoop(batalha, autor_id, jid, xp_ganho, moedas, isBoss) {
  ...
  for (const parceiro_id of parceiros) {
    ...
    db.adicionarMoedas(parceiro_id, isBoss ? moedas * 1000 : moedas);
    ...
    await enviar(jid, bloco('🤝 VITÓRIA EM EQUIPE! 【🏆】', [
      ...
      isBoss ? `⭐ XP: +${xp_ganho} | 💰 +${moedas} 🥈 Prata` : `⭐ XP: +${xp_ganho} | 💰 +${moedas} 🥉 Bronze`
    ]), [parceiro_id]);
  }
}
...
async function iniciarEncontroBoss(from, jid) {
  const j0 = db.getJogador(from);
  if (!j0) return enviar(jid, '❌ Você não tem personagem! Use /criar.');
  if (j0.morto) return enviar(jid, '❌ Você está morto! Use /renascer.');
  if (batalhaAtiva.has(from)) return enviar(jid, '❌ Você já está em batalha! Use /matar, /habilidade ou /fugir.');
  ...
  const boss = gerarBoss(j0.regiao);
...
async function processarTurnoBatalha(from, jid, acao) {
  ...
  if (acao === 'matar' || acao === 'atacar') {
    ...
    const dano_monstro = Math.max(tick_normal.pular_contra_ataque ? 0 : 3, dano_bruto_monstro - Math.floor(defesa_jogador * 0.5));
    ...
    if (monstro_morreu) {
      ...
      db.adicionarXP(from, xp_ganho);
      checarTituloImperador(from);
      db.adicionarMoedas(from, isBoss ? moedas * 1000 : moedas);
      if (isBoss) db.adicionarConquista(from, 'iniciado_boss');
    ...
    const fase_msg = (isBoss && faseAtualBoss(batalha.boss)?.msg) ? [`\n👑 _${faseAtualBoss(batalha.boss).msg}_`] : [];
