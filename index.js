...
const { suicidar, reviverPorNecromante, aprovarAcaoServo, liberarServo } = require('./death');
...
function textoCaminho() {
  const opcoes = [
    { intro: '_Você caminha por uma trilha estreita quando o caminho se divide em dois._', a: 'Seguir pela trilha principal', b: 'Cortar caminho pela vegetação densa' },
    { intro: '_Uma névoa cobre o chão. À frente, dois caminhos se abrem entre as rochas._', a: 'Ir pelo caminho mais curto e visível', b: 'Contornar por trás das rochas' },
    { intro: '_Você ouve um barulho estranho ecoando ao longe._', a: 'Investigar de onde vem o barulho', b: 'Ignorar e seguir seu rumo normalmente' },
    { intro: '_Pegadas estranhas marcam o chão à sua frente, seguindo em duas direções._', a: 'Seguir as pegadas maiores', b: 'Seguir as pegadas menores' },
  ];
...
function flavorMonstro(nome) {
  const n = (nome || '').toLowerCase();
  const grupos = [
    { chaves: ['lobo', 'wolf'], frases: ['_Um uivo cortou o silêncio antes dele saltar das sombras._', '_Você sentiu os olhos famintos dele antes de vê-lo._'] },
    { chaves: ['zumbi', 'zombie'], frases: ['_Um cheiro podre anunciou a chegada muito antes dele aparecer._', '_Ele se arrasta gemendo, faminto por carne fresca._'] },
    { chaves: ['esqueleto', 'skeleton'], frases: ['_Ossos rangeram no escuro — algo antigo despertou._'] },
    { chaves: ['goblin'], frases: ['_Uma risada aguda ecoou entre as pedras antes dele pular na sua frente._'] },
    { chaves: ['aranha', 'spider'], frases: ['_Teias grudaram no seu rosto — tarde demais pra perceber o perigo._'] },
    { chaves: ['fantasma', 'espectro', 'ghost'], frases: ['_O ar esfriou de repente, e uma forma translúcida surgiu._'] },
    { chaves: ['orc'], frases: ['_Passos pesados fizeram o chão tremer antes dele aparecer._'] },
    { chaves: ['guarda', 'soldado', 'cavaleiro'], frases: ['_Uma figura armada bloqueou seu caminho, hostil._'] },
    { chaves: ['bruxa', 'witch'], frases: ['_Um cheiro de ervas queimadas anunciou a presença dela._'] },
    { chaves: ['vampiro', 'vampire'], frases: ['_Uma sombra rápida demais pra ser humana passou por você._'] },
    { chaves: ['dragao', 'dragão', 'dragon'], frases: ['_O céu escureceu com a sombra de asas colossais._'] },
    { chaves: ['coelho', 'cachorro', 'gato', 'raposa', 'javali', 'coruja', 'lobo', 'aguia', 'águia', 'urso', 'tigre', 'leao', 'leão'], frases: ['_Um animal selvagem cruzou seu caminho, alerta._'] },
  ];
...
async function processarMensagem(msg) {
  ...
  if (!texto.startsWith(PREFIX)) {
    if (escolhaCaminho.has(from) && (texto.trim() === '1' || texto.trim() === '2')) {
      escolhaCaminho.delete(from);
      await iniciarEncontroBatalha(from, jid);
      return;
    }
...
${S.baixo}`;
  ...
  if (cmd === 'batalha') {
    ...
    if (escolhaCaminho.has(from)) return enviar(jid, `❌ Responda a escolha de caminho primeiro! Digite 1 ou 2.`);

    const caminho = textoCaminho();
    escolhaCaminho.set(from, true);
  ...
  if (cmd === 'itens') {
    ...
    if (sub.includes('pocao') || sub.includes('poção')) {
    ...
    if (sub.includes('especial')) {
      const linhas = ITENS_LOJA.filter(i => !IDS_POCOES.includes(i.id) && !i.exclusiva).map(i => `${i.nome} — 🪙 ${i.preco}`);
