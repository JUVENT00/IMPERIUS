...
const { verRanking, verRankingConquistas, verConquistas, verTitulos, verTitulosDisponiveis, usarTitulo, verMissoes, progredirMissao, coletarMissao, matarJogador, darItem, abencoarJogador, maldicionarJogador, eventoGlobal, statusBot, verRankingMoedas, verRankingXP, verRankingKills, verRankingPets, iniciarEventoAleatorio, participarEventoAleatorio, finalizarEventoAleatorio, statusEventoAleatorio } = require('./events');
...
${S.baixo}`;
  ...
  if (cmd === 'jogar') {
    ...
    if (r_velha.resultado) {
      ...
      if (r_velha.resultado === 'empate') {
        ...
        db.adicionarMoedas(vencedor_id, 50);
        texto_fim = `❌⭕ *FIM DE JOGO!*\n\n${tabuleiroVelhaTexto(jogo_velha_atual)}\n\n🏆 *${vencedor_j?.nome || '???'} venceu!* +50 Belarium`;
      }
  ...
  if (cmd === 'caçaniquel') {
    ...
    if (aposta_slot < 10) return enviar(jid, `❌ Aposta mínima: 10 Belarium. Use: /caçaniquel [valor]`);
    if ((j_slot.moedas || 0) < aposta_slot) return enviar(jid, `❌ Você não tem Belarium suficiente!`);

    db.adicionarMoedas(from, -aposta_slot);
    const r_slot = jogarCacaNiquel(aposta_slot);
    if (r_slot.ganho > 0) db.adicionarMoedas(from, r_slot.ganho);

    return enviar(jid, `🎰 [ ${r_slot.resultado.join(' | ')} ]\n\n${r_slot.ganho > 0 ? `🎉 *Você ganhou ${db.formatarPreco(r_slot.ganho)}!*` : '❌ *Não foi dessa vez...*'}`);
  }
  ...
  if (cmd === 'ppt') {
    ...
    if (r_ppt.resultado === 'vitoria') db.adicionarMoedas(from, 20);
  ...
  if (cmd === 'adivinhar') {
    ...
    if (r_adv.acertou) {
      ...
      const premio = Math.max(10, 100 - jogo_adv_atual.tentativas * 10);
      db.adicionarMoedas(from, premio);
  ...
  if (cmd === 'caracoroa') {
    ...
    if (aposta_cc > 0 && ((j_cc.moedas || 0) < aposta_cc || (alvo_j_cc.moedas || 0) < aposta_cc)) {
      return enviar(jid, `❌ Um dos dois não tem Belarium suficiente pra essa aposta!`);
    }
    ...
    if (r_cc.acertou && aposta_cc > 0) {
      db.adicionarMoedas(from, aposta_cc);
      db.adicionarMoedas(alvo_cc, -aposta_cc);
      texto_cc += `🎉 *${j_cc.nome} venceu!* +${db.formatarPreco(aposta_cc)}`;
    } else if (!r_cc.acertou && aposta_cc > 0) {
      db.adicionarMoedas(from, -aposta_cc);
      db.adicionarMoedas(alvo_cc, aposta_cc);
      texto_cc += `🎉 *${alvo_j_cc.nome} venceu!* +${db.formatarPreco(aposta_cc)}`;
    } else {
