...
${S.baixo}`;
  ...
  if (cmd === 'viajar') {
    ...
    // Suporte ao código >VXX
    if (resto.startsWith('>')) {
      const codigo = resto.slice(1).toUpperCase().trim();
      const MAPA_CODIGOS = {
        'VCS': 'ceu_flutuante', 'VTV': 'tundra_voryn', 'VCG': 'cavernas_gelo',
        'VPK': 'pico_kaldros', 'VST': 'serra_titas', 'VMD': 'montanha_dragao',
        'VFE': 'floresta_eryndal', 'VBS': 'bosque_sombras', 'VMP': 'mata_espiritos',
        'VRA': 'ruinas_aelthar', 'VPM': 'portal_mundos', 'VVC': 'valdris',
        'VIM': 'ilhas_marveth', 'VAM': 'abismo_mar_negro', 'VND': 'necropole_draktum',
        'VPL': 'pantano_maldito', 'VTC': 'torre_caos', 'VVI': 'vulcao_ignareth',
        'VPC': 'planicies_cinzas', 'VDA': 'deserto_aresh', 'VDE': 'dunas_esquecimento'
      };
      const regiao_real = MAPA_CODIGOS[codigo];
      if (!regiao_real) return enviar(jid, bloco('❌ CÓDIGO INVÁLIDO 【🗺️】', [
        `Código "${resto}" não existe!`,
        '━━━━━━━━━━',
        '/mapa — Ver todos os códigos'
      ]));
      return enviar(jid, viajar(from, regiao_real));
    }
