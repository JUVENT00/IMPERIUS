...
function batalharMonstro(jogador_id) {
  ...
  if (monstro_morreu) {
    ...
    logs.push(`📚 +${ganhos.xp} XP | 💰 +${formatarPreco(ganhos.moedas)}`);
  ...
  if (monstro_morreu) {
    ...
    registrarHistorico(jogador_id, `⚔️ Venceu uma batalha (+${ganhos.xp} XP)`);
...
function batalharBoss(jogador_id) {
  ...
  // boss.moedas já está na mesma escala "crua" (bronze-equivalente) que
  // monstro.moedas e xp — multiplicar por 1000 aqui inflava o prêmio real
  // (ex: 9.000 virava 9.000.000 = ~9 Ouro em vez de ~9 Prata), quebrando
  // a economia e os preços da loja.
  const boss_moedas = rand(boss.moedas[0], boss.moedas[1]);
  if (boss_morreu) {
    logs.push(`\n🏆 *BOSS DERROTADO!*`);
    logs.push(`📚 +${boss.xp} XP | 💰 +${formatarPreco(boss_moedas)}`);
  ...
  if (boss_morreu) {
    ...
    registrarHistorico(jogador_id, `🏆 Derrotou o boss ${boss.nome} (+${boss.xp} XP)`);
  }
...
module.exports = {
  batalharMonstro, batalharBoss, pvp, usarHabilidade, usarUltimate, usarHabilidadeArma, usarSupremaArma, rolarD20, rand, gerarMonstro,
