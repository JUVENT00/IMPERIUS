...
// ============================================================
const { getJogador, salvarJogador, adicionarXP, adicionarMoedas, adicionarConquista, adicionarTitulo, formatarBelarium } = require('./db');
...
function batalharMonstro(jogador_id) {
  ...
  if (monstro_morreu) {
    ...
    logs.push(`📚 +${ganhos.xp} XP | 💰 +${ganhos.moedas} 🥉 Bronze`);
...
function batalharBoss(jogador_id) {
  ...
  if (boss_morreu) {
    logs.push(`\n🏆 *BOSS DERROTADO!*`);
    logs.push(`📚 +${boss.xp} XP | 💰 +${boss_prata} 🥈 Prata`);
