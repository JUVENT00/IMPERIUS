...
function calcularDefesa(jogador) {
  const bonus = getBonusAtributo(jogador.xp);
  return Math.floor(((jogador.con || 10) + bonus) * 1.2);
}
...
function batalharBoss(jogador_id) {
  ...
  if (boss_morreu) {
    logs.push(`\n🏆 *BOSS DERROTADO!*`);
    logs.push(`📚 +${boss.xp} XP | 💰 +${rand(boss.moedas[0], boss.moedas[1])} moedas`);
    if (boss.drop_arma) logs.push(`⚔️ *Drop especial!* ${boss.drop}`);
    adicionarXP(jogador_id, boss.xp);
    adicionarMoedas(jogador_id, rand(boss.moedas[0], boss.moedas[1]));
...
module.exports = { batalharMonstro, batalharBoss, pvp, usarHabilidade, usarUltimate, rolarD20, rand, gerarMonstro };
