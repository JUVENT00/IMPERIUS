...
// ── SISTEMA DE ADM (poder limitado, concedido só pelo Dono) ──
// ADM só pode reviver jogadores mortos — nada de armas, itens ou moedas.
function listarADMs() {
  return getConfig('admins_revive') || [];
}

function isADM(jogador_id) {
  return listarADMs().includes(jogador_id);
}

function darADM(alvo_id) {
  const alvo = getJogador(alvo_id);
  if (!alvo) return { erro: '❌ Jogador não encontrado.' };
  const lista = listarADMs();
  if (lista.includes(alvo_id)) return { erro: `❌ *${alvo.nome}* já é ADM.` };
  lista.push(alvo_id);
  setConfig('admins_revive', lista);
  return { sucesso: true, msg: `⚖️ *${alvo.nome}* agora é ADM!\n_Pode usar /revivernpc pra trazer jogadores mortos de volta._` };
}

function removerADM(alvo_id) {
  const alvo = getJogador(alvo_id);
  if (!alvo) return { erro: '❌ Jogador não encontrado.' };
  const lista = listarADMs();
  if (!lista.includes(alvo_id)) return { erro: `❌ *${alvo.nome}* não é ADM.` };
  setConfig('admins_revive', lista.filter(id => id !== alvo_id));
  return { sucesso: true, msg: `⚖️ *${alvo.nome}* não é mais ADM.` };
}
...
module.exports = {
  ...
  listarADMs, isADM, darADM, removerADM,
