...
function chanceDomarAnimal(raridade) {
  return Math.max(10, 80 - (raridade - 1) * 12);
}
...
function tentarDomarAnimal(jogador_id, nome_monstro) {
  const j = getJogador(jogador_id);
  if (!j) return { erro: '❌ Personagem não encontrado!' };
  if (j.animal) return { erro: `❌ Você já tem um animal domado (${j.animal.nome})!\nUse /soltaranimal para soltar o atual.` };

  const [animal_id, animal] = identificarAnimal(nome_monstro);
  const chance = chanceDomarAnimal(animal.raridade);
  const roll = Math.random() * 100;

  if (roll > chance) {
    return {
      sucesso: false,
      texto: `${animal.emoji} *${animal.nome}* fugiu assustado!\n${barraEstrelas(animal.raridade)}\n_Chance que você tinha: ${chance}%_`
    };
  }

  j.animal = { id: animal_id, nome: animal.nome, raridade: animal.raridade, dano: animal.dano, cooldown: 0 };
  salvarJogador(jogador_id, j);
  return {
    sucesso: true,
    texto: `✅ *${animal.nome}* foi domado com sucesso!\n${barraEstrelas(animal.raridade)}\n_Chance que você tinha: ${chance}%_\n\nUse /chamarpet em batalha pra ele te ajudar!`
  };
}

// Chamar animal fora de batalha (apenas exibição/carinho)
function chamarPet(jogador_id) {
  const j = getJogador(jogador_id);
  if (!j) return { erro: '❌ Personagem não encontrado!' };
  if (!j.animal) return { erro: '❌ Você não tem um animal domado!\nTente domar um durante uma /batalha contra um animal selvagem.' };
  const animal = ANIMAIS[j.animal.id];
  return { texto: `${animal.emoji} *${j.animal.nome}* correu até você e esfregou a cabeça no seu braço!\n${barraEstrelas(j.animal.raridade)}` };
}

// Chamar animal DURANTE batalha, pra ele atacar (com cooldown de 30s pra não virar spam)
function chamarAnimalBatalha(jogador_id) {
  ...
  if (j.animal.cooldown && agora - j.animal.cooldown < 30 * 1000) {
    const restante = Math.ceil((30 * 1000 - (agora - j.animal.cooldown)) / 1000);
    return { erro: `⏳ ${j.animal.nome} ainda está descansando! Aguarde ${restante}s.` };
  }

  const [min, max] = j.animal.dano;
  const dano = Math.floor(Math.random() * (max - min + 1)) + min;
  j.animal.cooldown = agora;
  salvarJogador(jogador_id, j);

  const animal_data = ANIMAIS[j.animal.id];
  return { sucesso: true, dano, nome: j.animal.nome, emoji: animal_data?.emoji || '🐾' };
}
...
function verAnimais() {
  ...
  return `🐾 BESTIÁRIO DE ANIMAIS\n\n${linhas.join('\n')}\n━━━━━━━━━━\n_Encontre-os em /batalha e tente /tentardomar!_\n_Quanto mais estrelas, mais raro e mais difícil de domar._`;
}

function verMeuAnimal(jogador_id) {
  const j = getJogador(jogador_id);
  if (!j) return '❌ Personagem não encontrado!';
  if (!j.animal) return '❌ Você não tem um animal domado!\n\nEncontre um em /batalha e use /tentardomar.';
  const animal = ANIMAIS[j.animal.id];
  return `🐾 MEU ANIMAL\n\n${animal.emoji} ${j.animal.nome}\n${barraEstrelas(j.animal.raridade)}\n━━━━━━━━━━\n⚔️ Dano: ${animal.dano[0]}-${animal.dano[1]}\n━━━━━━━━━━\n_Use /chamarpet em batalha pra ele atacar!_`;
}
...
function adotarAnimal(jogador_id, nome_animal) {
  const j = getJogador(jogador_id);
  if (!j) return '❌ Personagem não encontrado!';
  if (j.animal) return `❌ Você já tem um animal domado (${j.animal.nome})!\nUse /soltaranimal para soltar o atual.`;
  ...
  j.animal = { id: animal_id, nome: animal.nome, raridade: animal.raridade, dano: animal.dano, cooldown: 0 };
  salvarJogador(jogador_id, j);
  return `✅ *${animal.nome}* adotado por ${PRECO_ADOCAO} 🪙!\n${barraEstrelas(animal.raridade)}\n\nUse /chamarpet em batalha pra ele te ajudar!`;
}
