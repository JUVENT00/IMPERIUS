...
// ============================================================
const { getJogador, salvarJogador, adicionarConquista, adicionarTitulo, getPendente, setPendente, deletePendente, calcularXPProximo, getRank } = require('./db');
...
function suicidar(jogador_id) {
  ...
  jogador.xp = 0;
  jogador.nivel = 1;
  jogador.xp_proximo = calcularXPProximo(1);
  jogador.rank = getRank(1);
  jogador.moedas = 0;
  jogador.inventario = [];
  jogador.arma = null;
  jogador.armadura = null;
  jogador.pet = null;
  jogador.animais = [];
  jogador.titulos = [];
  ...
  jogador.sorte_batalhas_restantes = 0;
  jogador.sorte_valor = 0;
  jogador.regiao = 'valdris';
  jogador.historico = [`${new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} — 💀 Tirou a própria vida (${nome_antigo})`];
