...
// ── RESET GERAL (apaga dados de jogadores) ─────────────────
// Sempre faz um backup ANTES de apagar, com nome fixo (não entra na
// rotação de limpeza automática dos backups_*), pra sempre dar pra
// restaurar manualmente se precisar. É async porque o backup real
// só termina quando a Promise resolve — apagar antes disso terminar
// deixaria o backup de segurança incompleto.
async function resetarTudo() {
  const backup_path = path.join(DATA_DIR, `pre_reset_${Date.now()}.db`);
  await db.backup(backup_path);

  const tabelas = ['jogadores', 'pendentes', 'sacrificios', 'encarnacoes', 'guildas', 'casamentos', 'evento_deus'];
  tabelas.forEach(t => db.prepare(`DELETE FROM ${t}`).run());
  db.prepare(`DELETE FROM config WHERE chave = 'leiloes_ativos'`).run();

  return backup_path;
}

// ── SISTEMA DE MOEDAS (Belarium: Bronze / Prata / Ouro) ────
// 1.000 Bronze = 1 Prata | 1.000 Prata = 1 Ouro
// `jogador.moedas` continua existindo (compatibilidade com o resto
// do bot) e representa sempre o TOTAL em bronze equivalente.
// `jogador.belarium = { bronze, prata, ouro }` é a estrutura nova,
// sempre mantida em sincronia com `moedas` nas funções abaixo.
function normalizarBelarium(j) {
  const total = Math.max(0, Math.floor(j.moedas || 0));
  const ouro = Math.floor(total / 1000000);
  const prata = Math.floor((total % 1000000) / 1000);
  const bronze = total % 1000;
  j.belarium = { bronze, prata, ouro };
  j.moedas = total; // garante que moedas nunca fica quebrado/negativo/decimal
  return j;
}
...
function getJogador(id) {
  ...
  if (!row) return null;
  return normalizarBelarium(JSON.parse(row.dados));
}

function salvarJogador(id, dados) {
  normalizarBelarium(dados); // outros arquivos mexem em jogador.moedas direto — resincroniza antes de salvar
  db.prepare(`
...
function todosJogadores() {
  return db.prepare('SELECT id, dados FROM jogadores').all().map(r => ({ ...JSON.parse(r.dados), _id: r.id }));
}
...
function adicionarXP(id, quantidade) {
  ...
  while (nivel < 200) {
    ...
    if (j.xp >= xp_necessario) {
      ...
      j.for += 1;
      j.con += 1;
      j.pontos_talento = (j.pontos_talento || 0) + 1;
  ...
  if (subiu) {
    ...
    j.historico.push(`${new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} — ⭐ Subiu para o nível ${j.nivel}`);
    if (j.historico.length > 50) j.historico = j.historico.slice(-50);
  }
...
function adicionarMoedas(id, quantidade) {
  const j = getJogador(id);
  if (!j) return;
  const bonus_talento = quantidade > 0 ? 1 + Math.min((j.talentos && j.talentos.moedas) || 0, 20) * 0.01 : 1;
  j.moedas = Math.max(0, (j.moedas || 0) + Math.floor(quantidade * bonus_talento));
  salvarJogador(id, j); // salvarJogador já resincroniza j.belarium
}

// Texto padrão "🥇1 🥈23 🥉456" pra usar em /perfil, /banco, mensagens de compra etc.
// Sempre chamar em cima de um jogador vindo de getJogador (já normalizado).
function formatarBelarium(jogador) {
  const b = jogador.belarium || { bronze: jogador.moedas || 0, prata: 0, ouro: 0 };
  const partes = [];
  if (b.ouro > 0) partes.push(`🥇${b.ouro}`);
  if (b.prata > 0 || b.ouro > 0) partes.push(`🥈${b.prata}`);
  partes.push(`🥉${b.bronze}`);
  return partes.join(' ');
}

// Formata qualquer valor cru (preço de loja, custo de craft, etc.) na quebra Ouro/Prata/Bronze
function formatarPreco(valorBronze) {
  const total = Math.max(0, Math.floor(valorBronze || 0));
  const ouro = Math.floor(total / 1000000);
  const prata = Math.floor((total % 1000000) / 1000);
  const bronze = total % 1000;
  const partes = [];
  if (ouro > 0) partes.push(`🥇${ouro}`);
  if (prata > 0 || ouro > 0) partes.push(`🥈${prata}`);
  partes.push(`🥉${bronze}`);
  return partes.join(' ');
}

function registrarHistorico(id, texto) {
  const j = getJogador(id);
  if (!j) return;
  if (!Array.isArray(j.historico)) j.historico = [];
  j.historico.push(`${new Date().toLocaleString('pt-BR', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' })} — ${texto}`);
  if (j.historico.length > 50) j.historico = j.historico.slice(-50);
  salvarJogador(id, j);
}
...
module.exports = {
  ...
  formatarBelarium, formatarPreco, normalizarBelarium, registrarHistorico,
  calcularXPProximo,
  getConfig, setConfig,
