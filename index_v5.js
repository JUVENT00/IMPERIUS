...
async function processarMensagem(msg) {
  ...
  const num = from.replace('@s.whatsapp.net','').replace('@lid','').split(':')[0];
...
${S.baixo}`;
    try { await sock.sendMessage(jid, { image: { url: 'https://raw.githubusercontent.com/JUVENTOO/IMPERIUS/main/menu.jpg' }, caption: menu_txt }); } catch { await sock.sendMessage(jid, { text: menu_txt }); }
    return;
  }
  // ── MENU OLD ──────────────────────────────────────────────
  ...
  if (cmd === 'mapa') {
    ...
    try {
      await sock.sendMessage(jid, {
        image: { url: 'https://raw.githubusercontent.com/JUVENTOO/IMPERIUS/main/mapa.jpg' },
        caption: mapa_txt
      });
    } catch {
...
async function processarCriacao(from, jid, texto, msg) {
  ...
  if (estado.etapa === 'historia') {
    ...
    return enviar(jid, bloco('𝐏𝐄𝐑𝐒𝐎𝐍𝐀𝐆𝐄𝐌 𝐂𝐑𝐈𝐀𝐃𝐎 【⚔️】', [
      `👤 ${nome}`,
      `🎭 ${classeData.nome}`,
      `📅 ${idade} anos`,
      `📖 _${historia}_`,
