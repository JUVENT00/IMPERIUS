...
// ============================================================
const { criarJogador, getJogador, salvarJogador, adicionarConquista, adicionarTitulo, formatarBelarium, getGuilda } = require('./db');
const { CLASSES, CLASSES_NORMAIS, CLASSES_RARAS, REGIOES, ARMAS, ARMADURAS, TITULOS } = require('./gameData');
// ── MENU DE CLASSES ────────────────────────────────────────
...
function girarRoleta(jogador_id) {
  ...
  if (!podeUsarRoleta(jogador)) {
    const restante = Math.ceil((10 * 60 * 1000 - (Date.now() - jogador.cooldown_roleta)) / 60000);
    return { erro: `⏳ Você já girou a roleta! Aguarde *${restante} min* para girar novamente.` };
  }
  ...
  // Aplicar classe (preservando a escala do nível atual, igual ao /setnivel usa —
  // sem isso, o HP máximo "reseta" pro valor base da classe e diverge do que
  // aparece durante a batalha, que já tinha o bônus de nível acumulado)
  const classeAnterior = jogador.classe;
  const niveis_ganhos = Math.max(0, (jogador.nivel || 1) - 1);
  jogador.classe = classe_key;
  jogador.hp_max = classeData.hp + niveis_ganhos * 10;
  jogador.hp = jogador.hp_max;
  jogador.mana_max = classeData.mana + niveis_ganhos * 5;
  jogador.mana = jogador.mana_max;
  jogador.for = classeData.for + niveis_ganhos;
  jogador.des = classeData.des;
  jogador.con = classeData.con + niveis_ganhos;
...
function nomePet(jogador) {
  if (!jogador.pet) return jogador.tem_dragao ? '🐉 Dragão (Dragomante)' : 'Nenhum';
  if (jogador.pet.nome_dado && jogador.pet.nome_dado.trim()) {
    return `${jogador.pet.especie} (${jogador.pet.nome_dado})`;
  }
  return jogador.pet.especie || jogador.pet.nome || 'Pet sem nome';
}
...
function gerarFicha(jogador) {
  ...
  const guilda = jogador.guilda_id ? getGuilda(jogador.guilda_id) : null;

  const barra_hp = gerarBarra(jogador.hp, jogador.hp_max);
  const barra_mana = gerarBarra(jogador.mana || 0, jogador.mana_max || classeData?.mana || 100);
...
⭐ Nível: *${jogador.nivel || 1}* | Rank: *${jogador.rank}* | 📚 XP: *${jogador.xp}/${jogador.xp_proximo || '?'}*

❤️ HP: *${jogador.hp}/${jogador.hp_max}*
${barra_hp}
💧 Mana: *${jogador.mana || 0}/${jogador.mana_max || classeData?.mana || 100}*
${barra_mana}
...
🛡️ Armadura: *${armaduraData?.nome || 'Nenhuma'}*
${armaduraData ? `   Defesa: +${armaduraData.defesa} | ${armaduraData.raridade}` : ''}
🐾 Pet: *${nomePet(jogador)}*

🗺️ Região: *${regiao?.nome || capitalizar(jogador.regiao)}*
🏛️ Guilda: *${guilda?.nome || 'Nenhuma'}*
💰 Belarium: ${formatarBelarium(jogador)}
...
function viajar(jogador_id, destino) {
  ...
  const nivel_estimado = jogador.nivel || 1;

  if (nivel_estimado < regiao.nivel_min) {
    return `⚠️ *${regiao.nome}* exige nível mínimo!\n━━━━━━━━━━\n📊 Seu nível: *${nivel_estimado}*\n⭐ Nível necessário: *${regiao.nivel_min}-${regiao.nivel_max}*\n━━━━━━━━━━\n_Ganhe mais XP batalhando (/batalha) antes de tentar viajar para cá._`;
  }
  ...
  return `🗺️ Você viajou para *${regiao.nome}*!\n\n⚠️ Nível recomendado: *${regiao.nivel_min}-${regiao.nivel_max}*\n👾 Inimigos: _${regiao.monstros.slice(0, 3).map(m => m.nome).join(', ')}..._\n💀 Boss: *${regiao.bosses?.[0]?.nome || 'Desconhecido'}*`;
}
...
// ── MAPA ──────────────────────────────────────────────────
// Antes listava as regiões na ordem em que foram escritas no gameData.js
// (ordem arbitrária, sem relação com nível), o que deixava o mural fora
// de ordem. Agora ordena sempre por nivel_min antes de exibir.
function regioesOrdenadas() {
  return Object.entries(REGIOES).sort((a, b) => (a[1].nivel_min || 0) - (b[1].nivel_min || 0));
}

function verMapa() {
  let texto = `🗺️ *MAPA DO IMPERIUS* 🗺️\n━━━━━━━━━━━━━━━━━━━━\n\n`;
  regioesOrdenadas().forEach(([key, r]) => {
    texto += `${r.nome} — Nível ${r.nivel_min}-${r.nivel_max}\n`;
  });
...
function verRegioes() {
  ...
  regioesOrdenadas().forEach(([key, r]) => {
    texto += `${r.nome}\n`;
    texto += `   Nível: ${r.nivel_min}-${r.nivel_max} | Boss: ${r.bosses?.[0]?.nome || 'Desconhecido'}\n\n`;
  });
  return texto;
}
