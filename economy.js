...
function verLoja() {
  ...
  texto += `\n🧪 *ITENS:*\n`;
  ITENS_LOJA.filter(i => i.preco > 0 && !i.exclusiva).forEach(i => {
...
function buscarPorNomeERaridade(lista, busca) {
  ...
  if (leftover) {
    ...
    if (filtrados.length) {
      ...
      // Tinha texto depois do nome, mas não bateu com nenhuma raridade real —
      // não dá pra saber qual o jogador queria, então NÃO escolhe sozinho.
      leftoverReconhecido = false;
    }
  }

  const nomesRestantes = new Set(candidatos.map(a => a.nome));
  // Ambíguo: mais de um item candidato e (a) são nomes diferentes (ex: "Adaga"
  // bateu em "Adaga Vingativo" E "Adaga Dourado"), ou (b) mesmo nome mas várias
  // raridades sem uma raridade válida informada pra desempatar.
  if (candidatos.length > 1 && (nomesRestantes.size > 1 || !leftover || !leftoverReconhecido)) {
...
function mensagemAmbiguidade(candidatos, busca) {
  const nomesDistintos = new Set(candidatos.map(a => a.nome)).size;
  const ordenados = candidatos.sort((a, b) => a.preco - b.preco);
  const MAX_LINHAS = 12;
  const linhas = ordenados.slice(0, MAX_LINHAS).map(a => {
    const stats = [
      a.dano ? `Dano: ${a.dano[0]}-${a.dano[1]}` : null,
      a.defesa ? `Defesa: +${a.defesa}` : null
    ].filter(Boolean).join(' | ');
    return `• *${a.nome}* ${a.raridade} — 💰 ${a.preco}${stats ? ` | ${stats}` : ''}`;
  });
  const sobrando = ordenados.length - linhas.length;

  const titulo = nomesDistintos > 1
    ? `❓ *"${busca}"* bateu com mais de uma arma diferente:`
    : `❓ *"${busca}"* tem várias raridades disponíveis:`;
  const dica = nomesDistintos > 1
    ? `📝 Digite o nome completo pra escolher, ex:\n*/comprar ${ordenados[0].nome.replace(/^[^\w]+/, '').trim()} ${ordenados[0].raridade.replace(/^[^\w]+/, '').trim()}*`
    : `📝 Inclua a raridade na busca pra escolher, ex:\n*/comprar ${busca} ${ordenados[0].raridade.replace(/^[^\w]+/, '').trim()}*`;

  return [
    titulo,
    ...linhas,
    ...(sobrando > 0 ? [`_...e mais ${sobrando} opção(ões). Seja mais específico._`] : []),
    '',
    dica
  ].join('\n');
}
...
function comprarItem(jogador_id, item_nome) {
  ...
  const item = ITENS_LOJA.find(i => i.preco > 0 && !i.exclusiva && (normalizarTexto(i.nome).includes(busca) || normalizarTexto(i.id).includes(busca)));
...
function resolverMetaInventario(id) {
  const item = ITENS_LOJA.find(x => x.id === id);
  if (item) return item;
  const arma = ARMAS.find(x => x.id === id);
  if (arma) return arma;
  const armadura = ARMADURAS.find(x => x.id === id);
...
function venderItem(jogador_id, item_nome) {
  ...
  const buscaLimpa = limparNome(item_nome);
  const buscaBruta = normalizarTexto(item_nome);

  // Mapeia cada slot do inventário pro seu item real (nome/raridade/preço)
  const candidatos = jogador.inventario
    .map((id, idx) => ({ id, idx, meta: resolverMetaInventario(id) }))
    .filter(c => {
      if (normalizarTexto(c.id).includes(buscaBruta)) return true;
      if (!c.meta) return false;
      return limparNome(c.meta.nome).includes(buscaLimpa);
    });

  if (candidatos.length === 0) return '❌ Item não encontrado no inventário. Use /inventario para ver o que você tem.';

  // Se a busca bate em várias cópias do MESMO nome (raridades diferentes do
  // mesmo item), evita vender a errada só porque foi a primeira encontrada —
  // mesmo problema que existia na compra.
  const nomesUnicos = new Set(candidatos.map(c => c.meta ? c.meta.nome : c.id));
  let escolhido = candidatos[0];
  if (candidatos.length > 1 && nomesUnicos.size === 1) {
    const nomeItem = candidatos[0].meta.nome;
    const leftover = buscaLimpa.replace(limparNome(nomeItem), '').trim();
    if (leftover) {
      const filtrados = candidatos.filter(c => c.meta && (raridadeNormalizada(c.meta.raridade).includes(leftover) || leftover.includes(raridadeNormalizada(c.meta.raridade))));
      if (filtrados.length) escolhido = filtrados[0];
      else return mensagemAmbiguidadeVenda(candidatos, item_nome);
    } else {
      return mensagemAmbiguidadeVenda(candidatos, item_nome);
    }
  }

  const idx = escolhido.idx;
  const item_key = jogador.inventario[idx];
  const item = ITENS_LOJA.find(i => i.id === item_key);
  ...
  else if (arma) { valor = Math.max(10, Math.floor((arma.preco || 50) * 0.4)); nome_exibido = `${arma.nome} ${arma.raridade}`; }
  else if (armadura) { valor = Math.max(10, Math.floor((armadura.preco || 50) * 0.4)); nome_exibido = `${armadura.nome} ${armadura.raridade}`; }
...
function mensagemAmbiguidadeVenda(candidatos, busca) {
  const linhas = candidatos
    .filter(c => c.meta)
    .sort((a, b) => a.meta.preco - b.meta.preco)
    .map(c => `• *${c.meta.nome}* ${c.meta.raridade} — venderia por ~${Math.max(10, Math.floor((c.meta.preco || 50) * 0.4))} 🥉`);
  return [
    `❓ Você tem *${candidatos.length}* itens chamados "${busca}" (raridades diferentes):`,
    ...linhas,
    '',
    '📝 Inclua a raridade na busca pra escolher, ex:',
    `*/vender ${busca} ${candidatos[0].meta ? candidatos[0].meta.raridade.replace(/^[^\w]+/, '').trim() : ''}*`
  ].join('\n');
}
