...
const { ITENS_LOJA, ARMAS, ARMADURAS, REGIOES, calcularNivel } = require('./gameData');
...
function usarItem(jogador_id, item_id) {
  ...
  switch (item.efeito) {
    ...
    case 'nivel_up': {
      const nivel_info = calcularNivel(jogador.xp || 0);
      const xp_faltante = Math.max(1, nivel_info.xp_proximo - nivel_info.xp_atual);
